/**
 * @file Root shell component — header, screen routing, modal orchestration,
 *       and the DocumentViewerModal.
 */
...
import { BottomBar, PressableScale } from "./components";
import { CustomerEditor, DocumentEditor, LoanEditor, PaymentEditor, VehicleEditor } from "./forms";
import {
            ...
            <View style={styles.viewerBackdrop}>
                <View style={styles.viewerContainer}>
                    ...
                    <View style={{ width: "100%", alignItems: "center", marginVertical: 12 }}>
                        {isImage ? (
                            <Image source={{ uri: document.fileUri }} style={styles.viewerImage} />
                        ) : (
                    ...
                    <View style={styles.viewerMeta}>
                        ...
                        <View style={styles.viewerMetaRow}>
                            ...
                            <Text style={styles.viewerMetaValue}>{document.documentScope}</Text>
                        </View>
                        {document.expiryDate ? (
                            <View style={styles.viewerMetaRow}>
                                <Text style={styles.viewerMetaLabel}>Expiry Date:</Text>
                                <Text style={styles.viewerMetaValue}>{document.expiryDate}</Text>
                            </View>
                        ) : null}
                        {document.notes ? (
                            <View style={styles.viewerMetaRow}>
...
/**
 * Root component — wires controller state into screens and modals.
 */
export function LedgerApp() {
    ...
    const openDefaultLoanModal = () => {
        ...
        const vehicleId = ledger.selectedCustomer?.vehicles[0]?.id ?? ledger.defaultVehicleId;
    ...
    const isDetailScreen = ledger.screen === "customer-detail" || ledger.screen === "loan-detail";
        ...
        <View style={styles.shell}>
            ...
            {ledger.screen === "customers" ? (
                <CustomersScreen
                    ...
                    onRefresh={ledger.handleRefresh}
                />
            ) : null}
            ...
            {showBottomBar ? (
                <BottomBar activeTab={ledger.screen} onTabPress={(tab) => ledger.setScreen(tab)} />
            ) : null}
